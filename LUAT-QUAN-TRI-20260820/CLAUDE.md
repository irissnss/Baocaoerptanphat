...
> WARNING **BẢN CHỤP MỘT THỜI ĐIỂM (20/08/2026) — HÃY KIỂM CÒN HẠN HAY KHÔNG.**
> **Mã băm BẢN GỐC trong kho riêng tư tại thời điểm chụp (SHA-256):** `0a1cb3bfabfb0cd346ba5fea2b4fa3553fae0e7623219fad8cf48d4d5350b5d8`
> Đây là bản chụp của 5 file quản trị (5 bản byte-identical). Nội dung dưới đây **khớp đúng bản gốc** tại thời điểm chụp.
> **Cách kiểm:** chạy `sha256sum` trên **file gốc trong kho riêng tư**. Khác mã trên → bản công bố này đã **LỖI THỜI**.
> *(Cố ý ghi mã của BẢN GỐC chứ không phải của chính file này — thêm dòng cảnh báo sẽ làm đổi mã của chính nó, thành ra không kiểm được.)*
> *(File trong kho báo cáo có thể bị đổi kiểu xuống dòng khi tải về, nên mã băm của CHÍNH file này sẽ khác bản gốc.)*
> **Bản này thay bản `LUAT-QUAN-TRI-20260819/`** — bản cũ GIỮ NGUYÊN, không xoá (`GOV-EDIT-PRESERVE-001`).
...
**SKILL REGISTRY SEARCH tra ở đâu:** `.governance/registry/skills.yml` — danh mục 125 skill, mỗi entry đủ 8 trường §L6. Sinh lại bằng `node scripts/tests/skills-registry-build.mjs`; kiểm bằng `npm run test:skills-registry`.
...
**Registry thật:** `.governance/registry/skills.yml` (đặt cạnh `tools.yml`, cùng convention). Cổng kiểm: `npm run test:skills-registry` — FAIL khi (a) số entry ≠ số thư mục skill, (b) entry thiếu trường bắt buộc, (c) entry trỏ slug không tồn tại, (d) thư mục skill không có entry.

**Nhãn trung thực bắt buộc:** trường không rút được từ nội dung thật của `SKILL.md` phải ghi `NEEDS_OWNER_INPUT` — **CẤM bịa** để lấp chỗ trống. `NO_USAGE_EVIDENCE` nghĩa là **không có dấu vết văn bản**, KHÔNG đồng nghĩa "chưa bao giờ dùng" (kho không ghi log kích hoạt skill) ⇒ **không** dùng số này làm căn cứ đề xuất xoá skill.
...
| **Trước khi CHỌN SKILL** cho một việc | `.governance/registry/skills.yml` — tra theo triệu chứng, kiểm **negative trigger** trước khi nạp (`GOV-SKILL-RESOLUTION-001` §L2/§L6). Cổng: `npm run test:skills-registry` |
...
| 20/08/2026 | `2.3` → `2.4` | Agent IDE (Owner duyệt 20/08/2026 — `PLAN-20260820-skill-library-closeout` D1) | Kiểm kê skill 19/08 phát hiện: §L2 bắt "SKILL REGISTRY SEARCH" nhưng `.governance/registry/` **không có file danh mục skill nào** ⇒ 125 skill (804 KB) không có cách tra hệ thống. Cổng `ref-exists-gate` không bắt được vì §L2 viết dạng **sơ đồ luồng**, không phải đường dẫn | **Registry mới:** `.governance/registry/skills.yml` (125 entry × 8 trường §L6 + slug/health_label/tracked/negative_trigger). **Cổng mới:** `npm run test:skills-registry` — 4 điều kiện FAIL, kiểm ngược 4/4 đạt, nối vào `test:gov-gates`. **Luật sửa:** §L2 + §L6 nêu đường dẫn thật + nhãn trung thực `NEEDS_OWNER_INPUT`; §V thêm 1 dòng chỉ mục; header lên 2.4 | **KHÔNG điều nào bị xoá.** Cả 4 điểm sửa đều là BỔ SUNG — dòng cũ giữ nguyên văn ở các hàng con ngay dưới |
| 20/08/2026 | — | Agent IDE | §L2 chỉ vẽ sơ đồ luồng, không nói tra danh mục Ở ĐÂU | — | **Dòng cũ nguyên văn (§L2, câu sau khối sơ đồ):**<br>`Chỉ audit toàn skill library khi task là Skill Governance/Audit.`<br>Dòng này **GIỮ NGUYÊN tại vị trí cũ**; đoạn trỏ đường dẫn được THÊM phía trên nó |
| 20/08/2026 | — | Agent IDE | §L6 liệt kê 8 trường phải index nhưng không nêu registry nằm ở đâu, cũng không có cổng kiểm | — | **Dòng cũ nguyên văn (§L6, câu cuối):**<br>`Mục tiêu: Owner không phải nhắc cùng một bài học nhiều lần.`<br>Dòng này **GIỮ NGUYÊN tại vị trí cũ**; 2 đoạn mới THÊM phía trên nó |
| 20/08/2026 | — | Agent IDE | §V chưa có dòng nào cho việc chọn skill | — | **Dòng cũ nguyên văn (§V chỉ mục, dòng bị đẩy xuống 1 hàng):**<br>`| Cần **theo dõi nợ kỹ thuật** | `.governance/registry/tech-debt.md` — **đọc TRƯỚC khi mở lại một vùng** … |`<br>Dòng này **GIỮ NGUYÊN**; dòng mới chèn phía TRÊN nó |
| 20/08/2026 | — | Agent IDE | Nâng 2.3 → 2.4 | — | **Dòng cũ nguyên văn (Doc Version, header):**<br>`> **DOC VERSION:** `2.3` — cập nhật 19/08/2026. Thay đổi: vá 3 lỗ cổng đếm `GOV-EDIT-PRESERVE-001` + thêm cổng phụ đếm mục file chuẩn + thêm trường `hạn_đóng` vào sổ nợ` |
